...
const ResetPassword = () => {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            ...
            navigate("/login")
            setNotification("Rebice su correo le llegara un link para restablecer su contraseña", "correcto", "productToCart")
            ...
            <form onSubmit={handleSubmit}>
                <div className="modal-reset-password">
                    <div className="email">
                        ...
                        {error === "Correo Electronico faltante" && <span className="error">{error}</span>}
                        {error === "Correo electrónico no válido" && <span className="error">{error}</span>}
                    </div>
                    <button onClick={handleResetPassword} className="btn-reset-password">Resetear Contraseña<i className="fa-solid fa-arrow-right-long"></i></button>
                </div>
            </form>
        </section>
    )
